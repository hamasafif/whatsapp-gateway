// server.js  (ES-module, env-based, X-Api-Key + JWT, register)
import 'dotenv/config';                      // baca .env
...
import crypto from 'crypto';                // untuk UUID
...
async function loadSecret() {
  ...
  if (!r.length) {
    ...
    await db.execute('INSERT INTO settings(k,v) VALUES(?,?) ON DUPLICATE KEY UPDATE v=?',
...
app.post('/login', async (req, res) => {
  ...
  if (!usr) return res.status(401).json({ error: 'invalid creds' });
  const secret = await loadSecret();
...
app.get('/device', auth, async (_, res) => {
  const [rows] = await db.execute('SELECT number,pushname,status,accesstoken FROM devices ORDER BY updated_at DESC LIMIT 1');
  res.json(rows.length ? rows[0] : { number: null, pushname: null, status: 'DISCONNECTED', accesstoken: null });
});
app.delete('/session', auth, async (req, res) => {
  try {
    if (sock?.user) await sock.logout();
    else if (sock?.ws) sock.end();
